...
async function renderPDF({ templateName, payload, locale, options = { base64: true } }) {
  const details = await getFormDetails(templateName)
  const templates = details.templates ?? []

  const langCode = locale ? locale.replace('-', '_').split('_')[0] : null
  const tpl = (langCode && templates.find(t => t.language === langCode)) ?? templates[0]

  const xdpTemplate = typeof tpl === 'string' ? tpl : tpl?.xdpTemplate
  const resolvedTemplateName = tpl?.templateName
